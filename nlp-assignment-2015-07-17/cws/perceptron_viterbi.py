...
def viterbi(words, classifier) :
    ...
    for i in range(len(words_tags)) :
        ...
        for j in range(4) :
            ...
            if (i != 0) :
                for k in range(4) :
                    prev_tag = classifier.labels[k]
                    X = classifier.extract_feature(words_tags, i, prev_tag, False)
                    score = scores[i - 1][k] + classifier._score(X, j)
                    if (score > pred_score) :
                        pos = k
                        pred_score = score
            else :
                prev_tag = "<s>"
                X = classifier.extract_feature(words_tags, i, prev_tag, False)
                pred_score = classifier._score(X, j)
    ...
    pos = 0
    for j in range(1,4) :
        if scores[i][j] > scores[i][pos] :
            pos = j
    result = []
    while i >= 0 :
        result.append(classifier.labels[pos])
        pos = paths[i][pos]
        i -= 1
    result.reverse()
    return result
...
file = codecs.open("final-test.2.txt","r","utf-8")
write_file = codecs.open("final-result-2.txt","w","utf-8")
try :
    for line in file.readlines() :
        line = line.strip("\n")
        tags = viterbi(line, perceptron)
        #print tags
        #break
        result = ""
